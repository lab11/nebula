...
#if NRF_MODULE_ENABLED(GALAXY_BLE)  

/*******************************************************************************
 *   STATIC AND GLOBAL VARIABLES
 ******************************************************************************/
static const galaxy_ble_config_t* ble_config;

galaxy_ble_app_t app;
...
#ifdef ENABLE_DFU
static galaxy_ble_service_t dfu_service = {
    .uuid128 =  {{0x23, 0xD1, 0xBC, 0xEA, 0x5F, 0x78, 0x23, 0x15,
                  0xDE, 0xEF, 0x12, 0x12, 0x00, 0x00, 0x00, 0x00}},
    .uuid_handle.uuid = BLE_DFU_SERVICE_UUID};

static galaxy_ble_char_t    dfu_ctrlpt_char = {.uuid16 = BLE_DFU_CTRL_PT_UUID};
...
static void on_adv_evt(ble_adv_evt_t ble_adv_evt) {
    // do nothing //TODO we probably want to do something 
}
...
static void on_ble_evt(ble_evt_t const * p_ble_evt, void * p_context) {
    ...
    switch (p_ble_evt->header.evt_id)
    {
        ...
        case BLE_GATTS_EVT_WRITE: {
            ...
            if (galaxy_ble_is_char_event(p_ble_evt, &dfu_ctrlpt_char)) {
                pending_dfu = 1;
...
void __attribute__((weak)) dfu_init (void) {
    ...
    galaxy_ble_add_service(&dfu_service);
    // Add ctrl pt characteristic
    galaxy_ble_add_characteristic(0,1,1,1,
...
galaxy_ble_app_t* galaxy_ble_init(const galaxy_ble_config_t* conf) {
    ble_config = conf;
    ...
    //galaxy_ble_device_info_service_automatic();
#endif
...
void galaxy_ble_add_service(galaxy_ble_service_t* service_handle) {
    uint32_t err_code;
...
void galaxy_ble_add_characteristic(uint8_t read, uint8_t write, uint8_t notify, uint8_t vlen,
                                   uint16_t len, uint8_t* buf,
                                   galaxy_ble_service_t* service_handle,
                                   galaxy_ble_char_t* char_handle) {
    ret_code_t err_code;
...
uint32_t galaxy_ble_update_char_len(galaxy_ble_char_t* char_handle, uint16_t len) {
...
uint32_t galaxy_ble_notify_char(galaxy_ble_char_t* char_handle) {
    volatile uint32_t err_code;
...
bool galaxy_ble_is_char_event(ble_evt_t const* p_ble_evt, galaxy_ble_char_t* char_handle) {
    ble_gatts_evt_write_t const* p_evt_write = &(p_ble_evt->evt.gatts_evt.params.write);

    return (bool)(p_evt_write->handle == char_handle->char_handle.value_handle);
}

void galaxy_ble_add_auth_characteristic(uint8_t read, uint8_t write, uint8_t notify, uint8_t vlen,
...
bool galaxy_ble_is_read_auth_event(ble_evt_t* p_ble_evt, galaxy_ble_char_t* char_handle) {
...
bool galaxy_ble_is_write_auth_event(ble_evt_t* p_ble_evt, galaxy_ble_char_t* char_handle) {
    ble_gatts_evt_rw_authorize_request_t* p_auth_req =
            &(p_ble_evt->evt.gatts_evt.params.authorize_request);
...
int32_t galaxy_ble_grant_auth(ble_evt_t* p_ble_evt) {
    ble_gatts_evt_rw_authorize_request_t* p_auth_req =
            &(p_ble_evt->evt.gatts_evt.params.authorize_request);
...
void galaxy_ble_add_stack_characteristic(uint8_t read, uint8_t write, uint8_t notify, uint8_t vlen,
                                          uint16_t len, uint8_t* buf,
                                          galaxy_ble_service_t* service_handle,
                                          galaxy_ble_char_t* char_handle) {
    volatile uint32_t err_code;
...
uint32_t galaxy_ble_stack_char_get(galaxy_ble_char_t* char_handle, uint16_t* len, uint8_t* buf) {
...
uint32_t galaxy_ble_stack_char_set(galaxy_ble_char_t* char_handle, uint16_t len, uint8_t* buf) {
    ble_gatts_value_t value = {
            .len = len,
            .offset = 0,
            .p_value = buf,
...
void galaxy_ble_adv_only_name(void) {
    ret_code_t err_code;
...
void galaxy_ble_adv_certificate(uint8_t* buf, size_t len) {
  // Set up an advertisement

  //uint8_t* cert = 12;  //actually add a cert instead of random things....
  //size_t len = sizeof(cert);
  //static uint8_t payload[24] = {0}; // 24 bytes are available for arbitrary data
  //if (len > 24) {
  //  len = 24;
  //}
  //memcpy(payload, cert, len);
  ...
  static ble_advdata_manuf_data_t adv_payload = {
    .company_identifier = 0x02E0, // Lab11 ID 
    .data.p_data = payload,
    .data.size = 24,
  };
  ...
  galaxy_ble_set_adv(&advdata, NULL);

}

void galaxy_ble_adv_manuf_data(uint8_t* buf, size_t len) {
  // Set up an advertisement
  static uint8_t payload[24] = {0}; // 24 bytes are available for arbitrary data
  if (len > 24) {
    len = 24;
...
void galaxy_ble_set_adv(ble_advdata_t* adv_data, ble_advdata_t* scan_rsp_data) {
    uint32_t err_code;
...
void galaxy_ble_es_adv(const char* url_str, const ble_advdata_t* scan_rsp_data) {
    uint32_t err_code;
...
void galaxy_ble_es_with_manuf_adv(const char* url_str, ble_advdata_manuf_data_t* manuf_specific_data) {
    ...
    galaxy_ble_es_adv(url_str, &srdata);
}

void galaxy_ble_es_with_name(const char* url_str) {
