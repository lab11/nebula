#ifndef __GALAXY_BLE_H
#define __GALAXY_BLE_H
...
typedef struct galaxy_ble_service_s {
    ...
    uint8_t     nr_sr_uuids;
} galaxy_ble_service_t;

typedef struct galaxy_ble_char_s {
    uint16_t uuid16;
...
/* Set advertising data to only advertise certificate
 *  buf: buffer to data to add to advertisement
 *  len: length of buffer. Note that only 24 bytes are available after header and flags.
 */
